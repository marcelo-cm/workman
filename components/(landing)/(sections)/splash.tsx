...
import { DecorativeGrid } from '../(molecules)/decorative-grid';
import {
...
const Landing = memo(() => {
  return (
    <span className="relative">
      <div className="container relative z-50 mx-auto flex h-[90dvh] flex-col items-center overflow-hidden p-12 px-4 sm:h-[80dvh] md:px-0 md:py-16 lg:h-[75dvh] lg:max-[1500px]:h-[65dvh] min-[1280px]:overflow-visible">
        ...
        <h1 className="fade-in mb-4 max-w-screen-lg text-balance text-center font-poppins text-3xl font-medium leading-[120%] sm:text-4xl md:mb-8 md:text-5xl lg:text-7xl ">
      ...
      <DecorativeGrid className="absolute left-0 top-0 z-0" />
      <DecorativeGrid className="absolute bottom-0 right-0 z-0 rotate-180" />
    </span>
  );
});
...
const TestimonialDetail = memo(
  ({ badge, content }: { badge: string; content: string }) => {
    return (
...
const Details = memo(() => {
  return (
    <div className="grid w-full grid-cols-1 gap-8 bg-zinc-800 p-8 sm:grid-cols-2 lg:max-h-[650px] lg:grid-cols-4 min-[1280px]:grid-rows-2">
      ...
      <TestimonialDetail
        badge="Pilot Program"
        content="We've partnered with a select group of homebuilders to test our
